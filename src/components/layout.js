    ...
    return <Row gutter={[16, 16]} style={{overflow: 'scroll', padding: 8,margin: 0, width: '100%', maxHeight: 'calc(100vh - 112px)'}}>
        {
            ...
            ?
            viewedProfiles.map(({liked, imgUrl, lastName}) => {
                const isShow = viewSelected === 'favorites' ? liked: !liked
                return isShow ?
                    <Col span={8} style={{height: 120}}>
            ...
            :
            <Empty style={{width: '100%', padding: '72px 0'}}/>
        }
    </Row> 
}